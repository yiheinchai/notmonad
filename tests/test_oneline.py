...
from examples.oneline import app, reset_db, seed
...
def test_oneline_home_lists_seeded_posts():
    response = _req("get", "/")
...
def test_oneline_post_detail_and_404():
    ok = _req("get", "/posts/1")
...
def test_oneline_create_post_then_see_it_on_home():
    created = _req(
        "post",
        "/posts/new",
...
def test_oneline_login_sets_cookie_and_admin_requires_auth():
...
def test_oneline_wsgi_get_home():
    captured = []
