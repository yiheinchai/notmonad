...
class TestLoopFunc:
    ...
    def test_loop_with_merge_state_on_real_world_data(self):
        test_data = [
            {
                "name": "Leanne Graham",
                "addresses": [
        ...
        assert (
            answer
            == monad(test_data, compose(debug, maybe))(
                monad(
                    lambda address: address["zipcode"][:5],
                    compose(debug, swap_val, maybe),
                )(partial, loop, v_key="map")(
                    pouterwrap, lambda x: x["addresses"], v_key="wrapper"
                )(
                    pinnerwrap, lambda x: {"address": x}, v_key="wrapper"
                    ...
                    state, pinnerwrap, lambda x: {"phone": x}, m_key="wrapper"
                )(
                    state
                )(
                    partial, loop, v_key="map"
